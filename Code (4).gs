 ...
 *
 *  SINKRONISASI EDIT LANGSUNG DI SHEET -> WEB:
 *  Karena doGet/doPost selalu membaca sheet langsung (tanpa cache), edit
 *  manual di Google Sheets OTOMATIS akan terbawa ke web setiap kali web
 *  mengambil data. Supaya web tahu KAPAN harus mengambil data baru tanpa
 *  harus reload seluruh data terus-menerus, project ini memakai "penanda
 *  versi" (LAST_EDIT_TS) yang di-update setiap kali ada perubahan sel di
 *  Sheet (lewat trigger onEdit) maupun perubahan lewat form web (addRow/
 *  updateRow/deleteRow). index.html hanya polling versi ini tiap beberapa
 *  detik (ringan, tidak membaca seluruh sheet) dan baru menarik data penuh
 *  kalau versinya berubah.
 *
 *  WAJIB DIJALANKAN SEKALI SETELAH PASTE SCRIPT INI (supaya edit oleh
 *  SIAPA PUN di Sheet -- bukan cuma pemilik script -- ikut terdeteksi):
 *    1. Di editor Apps Script, pilih fungsi "setupEditTrigger" pada
 *       dropdown fungsi di toolbar atas, lalu klik "Run".
 *    2. Berikan izin (authorize) saat diminta.
 *    3. Selesai — cukup dilakukan SEKALI per Google Sheet/project.
 * ============================================================================
 */

// Nama Script Property tempat menyimpan "versi" data terakhir kali berubah.
var SYNC_VERSION_PROP = 'LAST_EDIT_TS';
...
// ----------------------------------------------------------------------------
// SINKRONISASI: PENANDA VERSI DATA (dipakai frontend untuk auto-refresh)
// ----------------------------------------------------------------------------

/** Update penanda versi. Dipanggil dari trigger onEdit (edit manual di Sheet)
 *  DAN dari addRow_/updateRow_/deleteRow_ (edit lewat form web), supaya semua
 *  jenis perubahan data — dari mana pun sumbernya — kekbaca oleh polling. */
function bumpSyncVersion_() {
  try {
    PropertiesService.getScriptProperties().setProperty(SYNC_VERSION_PROP, String(new Date().getTime()));
  } catch (err) {
    // abaikan; kegagalan update versi tidak boleh menggagalkan operasi utama
  }
}

function getSyncVersion_() {
  var v = PropertiesService.getScriptProperties().getProperty(SYNC_VERSION_PROP);
  return v ? Number(v) : 0;
}

/**
 * SIMPLE TRIGGER bawaan Apps Script: otomatis berjalan tiap kali ada sel
 * yang diedit LANGSUNG di Google Sheets (oleh siapa pun yang membuka
 * spreadsheet). Karena simple trigger kadang dibatasi otorisasinya untuk
 * kolaborator yang belum pernah authorize script ini, function INI hanya
 * sebagai fallback ringan. Sumber utama deteksi edit yang RELIABLE untuk
 * semua kolaborator adalah installable trigger "onEditInstallable" (lihat
 * setupEditTrigger() di bawah) — WAJIB dijalankan sekali (lihat komentar
 * di kepala file).
 */
function onEdit(e) {
  bumpSyncVersion_();
...
/** Handler untuk installable trigger. Isinya sama dengan onEdit(), tapi
 *  karena dipasang lewat ScriptApp.newTrigger(), ia berjalan dengan
 *  otorisasi PEMILIK script — sehingga edit oleh kolaborator MANA PUN
 *  tetap terdeteksi, tidak hanya edit oleh pemilik. */
function onEditInstallable(e) {
  bumpSyncVersion_();
}

/**
 * JALANKAN SEKALI SAJA (lihat instruksi di kepala file) untuk memasang
 * installable trigger di atas. Aman dijalankan berkali-kali — trigger
 * lama dengan handler yang sama akan dihapus dulu supaya tidak dobel.
 */
function setupEditTrigger() {
  ...
  ScriptApp.newTrigger('onEditInstallable')
    ...
    .onEdit()
    .create();
  Logger.log('Trigger onEditInstallable berhasil dipasang untuk: ' + ss.getName());
}
...
function routeAction_(action, payload) {
  switch (action) {
    case 'getSyncVersion':
      // Endpoint ringan (tidak menyentuh isi sheet) untuk dipolling frontend
      // setiap beberapa detik guna mendeteksi ada-tidaknya perubahan data.
      return { success: true, version: getSyncVersion_() };
    case 'getAllPO':
...
function getDashboardData_(upcomingDays) {
  ...
  poRows.forEach(function (r) {
    ...
    if (cat === 'At Risk' || cat === 'Delay') {
      ...
      var fcst = parseDateSafe_(r['Delivery (Forecast)']);
      var delayDays = plan && fcst && fcst.getTime() > plan.getTime() ? diffDays_(fcst, plan) : 0;
      criticalItems.push({
  ...
  var progressActual = lastActualRow ? Number(lastActualRow.Actual_Pct) : Math.round(avgProgress * 100);
  var progressPlanned = lastActualRow ? Number(lastActualRow.Planned_Pct) : progressActual;
  var progressVariance = Math.round((progressActual - progressPlanned) * 10) / 10;

  // ---------- Warnings (disintesis dari PO Delay & Action lewat due date) ----------
  var warnings = [];
  poRows.forEach(function (r) {
    if (r['Category'] === 'Delay') {
      warnings.push({
